...
mats$intercept_LOO <- matrix(NA, nrow=length(proteins), ncol=length(tissues))
for(t.idx in 1:length(tissues)){
  mats$intercept_LOO[,t.idx] <- apply(mats$prots[, -t.idx], 1, mean, na.rm=T)
}
corMats[["intercept_LOO"]] <- sapply(1:length(tissues), function(t1){cor(mats$prots[,t1], mats$intercept_LOO[,t1], method="spearman", use="pairwise.complete.obs")})
...
ggsave("4_1_FinalFigure.pdf", width=7, height=5)
...
ggplot(pDat, aes(x=L1, y=value)) + geom_violin(fill = "lightgrey") + 
  ...
  theme_bw(24)
ggsave("4_2_AcrossSamples.pdf", width=7, height=5)
...
xx <- data.frame(
  ...
  overfit_cor_MSE = apply((mats$prots-mats$ratio_LOO)**2, 1, function(row){return(mean(row, na.rm=T))}),
...
ggplot(xx, aes(y=overfit_effect, group=cnt, x=factor(cnt))) + geom_boxplot() + 
  ...
  theme_bw(24)
ggsave("4_2_OverfitEffect.pdf", height=5, width=8)
...
# OVERFIT EFFECT ----------------------------------------------------------
names(mats)
mses <- lapply(mats[c(1,3,4,5)], function(m){
  x <- (m - mats$prots)**2
