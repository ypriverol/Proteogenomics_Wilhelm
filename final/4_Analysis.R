...
ggplot(xx, aes(y=overfit_effect, group=cnt, x=factor(cnt))) + geom_boxplot() + 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw(24)
ggsave(paste0("4_OverfitEffect",label,".pdf"), height=5, width=5)
...
# }))
sapply(mses, function(v){sum(v==0,na.rm=T)})
sapply(mses, function(v){sum(v < 1e-20,na.rm=T)})

matriXes2 <- c("Ratio", "Control")
str(msesPlot <- melt(mses[matriXes2]))
msesPlot$L1 <- factor(msesPlot$L1, levels=matriXes2)
ggplot(msesPlot, aes(x=L1, y=value)) + geom_boxplot() + 
...
matriXes3 <- c("Ratio", "RatioLOO")
str(msesPlot <- melt(mses[matriXes3]))
msesPlot$L1 <- factor(msesPlot$L1, levels=matriXes3)
msesPlot$L1 <- factor(msesPlot$L1, levels = matriXes3)
ggplot(msesPlot, aes(x=L1, y=value)) + geom_boxplot() + 
  scale_y_log10() + theme_bw(24) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab("MSE") + xlab("Method")
ggsave(paste0("4_MSE_LOO",label,".pdf"), height=5, width=7)
sapply(mses, median, na.rm=T)
