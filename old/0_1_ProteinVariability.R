...
# str(tissues <- colnames(geneUN))
# 
# # PROTEIN PREDICTION -------------------------------------------------------------------
# 
# str(protPred <- matrix(NA, nrow=length(proteins), ncol=length(tissues)))
# rownames(protPred) <- proteins
# colnames(protPred) <- tissues
# for(p.idx in 1:length(proteins)){
#   ratio <- median(protUN[p.idx, ]/geneUN[p.idx, ], na.rm=T)
#   for(t.idx in 1:length(tissues)){
#     protPred[p.idx, t.idx] <- geneUN[p.idx, t.idx] * ratio
#   }
# }
# sum(is.na(protPred))
# sum(is.na(geneUN))
# 
# str(nikProtPred <- matrix(NA, nrow=length(proteins), ncol=length(tissues)))
# rownames(nikProtPred) <- proteins
# colnames(nikProtPred) <- tissues
# for(p.idx in 1:length(proteins)){
#   for(t.idx in 1:length(tissues)){
#     ratio <- median(protUN[p.idx, -t.idx]/geneUN[p.idx, -t.idx], na.rm=T)
#     nikProtPred[p.idx, t.idx] <- geneUN[p.idx, t.idx] * ratio
#   }
# }
# sum(is.na(nikProtPred))
# 
# save(protPred, nikProtPred, proteins, tissues, geneUN, protUN, file="0_1_data.RData")

load("0_1_data.RData")

library(ggplot2)
library(reshape)
# Data exploration of the original matrix ---------------------------------
barplot(table(apply(protUN, 1, function(r) sum(!is.na(r)))), las  = 2, main = "Values per protein")
# ggplot(data.frame(count=apply(protUN, 1, function(r) sum(!is.na(r))))) + geom_bar(stat="bin", aes(x=count))
dev.print(pdf, "0_1_Proteins.pdf")
barplot(table(apply(geneUN, 1, function(r) sum(!is.na(r)))), las = 2, main = "Values per gene")
dev.print(pdf, "0_1_Transcripts.pdf")


# EXPLORE THESE DATA IN RELATIONSHIP --------------------------------------
xx <- data.frame(
  average = apply(protUN, 1, function(row) return(mean(row, na.rm=T))),
  claimed_MSE = apply((protUN-protPred)**2, 1, function(row){return(mean(row, na.rm=T))}),
  overfit_cor_MSE = apply((protUN-nikProtPred)**2, 1, function(row){return(mean(row, na.rm=T))}),
  var_prot = apply(protUN, 1, function(row){var(row, na.rm=T)}),
  var_gene = apply(geneUN, 1, function(row){var(row, na.rm=T)}),
...
# quantile(xx$overfit_effect <- (xx$overfit_cor_MSE - xx$claimed_MSE)/xx$claimed_MSE, na.rm=T)
quantile(xx$overfit_effect <- (xx$overfit_cor_MSE/xx$claimed_MSE), na.rm=T)
table(xx$cnt <- pmin(xx$cnt_prot, xx$cnt_gene))
table(xx$cnt_prot, xx$cnt_gene)
sum(xx$cnt_prot==0 | xx$cnt_gene == 0) * 12
...
ggplot(xx, aes(y=var_prot/average, x=average)) + geom_point() + theme_bw(24) + scale_y_log10() + scale_x_log10()
...
dev.print(pdf, "0_1_Variability_Correlation.pdf")
...
dev.print(pdf, "0_1_Variability_Correlation_WrongTissue.pdf")